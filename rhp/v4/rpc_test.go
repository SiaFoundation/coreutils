...
func TestPools(t *testing.T) {
	...
	res1, err := rhp4.RPCReplenishPools(context.Background(), transport, rhp4.RPCReplenishPoolsParams{
	...
	revision.Revision = res1.Revision
	res2, err := rhp4.RPCReplenishPools(context.Background(), transport, rhp4.RPCReplenishPoolsParams{
	...
	revision.Revision = res2.Revision
	seeded, err := c.PoolBalances([]proto4.Account{pool1, pool2})
	if err != nil {
		t.Fatal(err)
	}
	if !seeded[0].Equals(types.Siacoins(10)) {
		t.Fatalf("pool1 after seed: expected 10 SC, got %v", seeded[0])
	}
	if !seeded[1].Equals(types.Siacoins(20)) {
		t.Fatalf("pool2 after seed: expected 20 SC, got %v", seeded[1])
	}
	...
	if err := rhp4.RPCAttachPools(context.Background(), transport, []rhp4.PoolAttachInput{
		...
		t.Fatalf("expected ErrPoolNotFound attaching to non-existent pool, got %v", err)
	}
...
func TestPoolAttachAuth(t *testing.T) {
	...
	res, err := rhp4.RPCReplenishPools(context.Background(), transport, rhp4.RPCReplenishPoolsParams{
		Pools:    []proto4.Account{pool},
		Target:   types.Siacoins(5),
		Contract: revision,
	}, cm.TipState(), fundAndSign)
	if err != nil {
		t.Fatal(err)
	}
	...
	}, -time.Second); !errors.Is(err, proto4.ErrPoolExpired) {
		t.Fatalf("expected ErrPoolExpired attaching with past ValidUntil, got %v", err)
	}
...
func TestPoolBatchAttachDetach(t *testing.T) {
	...
	// drain ordering below depends on each pool starting at i+1 SC.
	for i, p := range pools {
		res, err := rhp4.RPCReplenishPools(context.Background(), transport, rhp4.RPCReplenishPoolsParams{
			Pools:    []proto4.Account{p},
			Target:   types.Siacoins(uint32(i + 1)),
			Contract: revision,
		}, cm.TipState(), fundAndSign)
		if err != nil {
			t.Fatal(err)
...
func TestPoolDebitAtomicOnInsufficient(t *testing.T) {
	...
	poolFund, err := rhp4.RPCReplenishPools(context.Background(), transport, rhp4.RPCReplenishPoolsParams{
		Pools:    []proto4.Account{pool},
		Target:   types.Siacoins(3),
		Contract: revision,
	}, cm.TipState(), fundAndSign)
	if err != nil {
		t.Fatal(err)
...
func TestPoolBackedReadWriteSector(t *testing.T) {
	...
	fundResult, err := rhp4.RPCReplenishPools(context.Background(), transport, rhp4.RPCReplenishPoolsParams{
		Pools:    []proto4.Account{pool},
		Target:   poolFundAmount,
		Contract: revision,
	}, cm.TipState(), fundAndSign)
	if err != nil {
		t.Fatal(err)
