...
func SeedFromPhrase(seed *[32]byte, phrase string) error {
	var entropy [16]byte
	if err := decodeBIP39Phrase(&entropy, phrase); err != nil {
		return err
	}
...
func decodeBIP39Phrase(entropy *[16]byte, phrase string) error {
	// validate that the phrase is well formed and only contains words that
	// are present in the word list
	words := strings.Fields(phrase)
	if n := len(words); n != 12 {
		return errors.New("wrong number of words in seed phrase")
	}
	for _, word := range words {
		if _, ok := wordMap[word]; !ok {
			return fmt.Errorf("unrecognized word %q in seed phrase", word)
		}
	}
