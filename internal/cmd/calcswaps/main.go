...
var expiringFileContractOrder = map[types.BlockID][]types.FileContractID\{
{{- range $id, $order := .}}
...
\}`)
	if err != nil {
		log.Panic("failed to parse template", zap.Error(err))
	}
	...
	if err := tmpl.Execute(f, overwriteIDs); err != nil {
		log.Panic("failed to execute template", zap.Error(err))
